...
import CommandParser from '../../../src/assets/scripts/client/commands/parser/CommandParser';
...
ava('sets #commandList with a Command object when provided a system command', t => {
    const model = new CommandParser(TIMEWARP_50_MOCK);

    t.true(model.commandList.length === 1);
    t.true(model.commandList[0] instanceof Command);
});

ava('sets #commandList with Command objects when it receives transmit commands', t => {
    const commandStringMock = buildCommandString(CAF_MOCK, CVS_MOCK, TAKEOFF_MOCK);
    const model = new CommandParser(commandStringMock);
...
ava('._extractCommandsAndArgs() calls _buildCommandList() when provided transmit commands', t => {
    ...
    model._parse(commandStringMock);

    t.true(_buildCommandListSpy.calledWithExactly(_tail(expectedArgs)));
});

ava('._buildCommandList() returns an empty array when adding args to an undefined Command', t => {
